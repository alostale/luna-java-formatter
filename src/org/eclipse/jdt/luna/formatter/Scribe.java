...
public class Scribe implements IJavaDocTagConstants {
	...
	Scribe(CodeFormatterVisitor formatter, long sourceLevel, IRegion[] regions,
	...
	/**
	 * This method will adapt the selected regions if needed. If a region should be
	 * adapted (see isAdaptableRegion(IRegion)) retrieve correct upper and lower
	 * bounds and replace the region.
	 */
	private void adaptRegions() {
		...
		for (int i = 0; i < max; i++) {
			...
			if (index >= 0) {
				// the offset of the region is inside a comment => restart the region from the
				// comment start
	 ...
	 * are outside the region: before when overlapping region's start and after when
	 * overlapping region's end. Note that the trailing indentation of the
	 * replacement string is not kept when the region's end is overlapped because
	 * it's always outside the region.
	 */
	private int adaptEdit(OptimizedReplaceEdit[] sortedEdits, int start, int regionStart, int regionEnd) {
		...
		while (bottom <= top) {
			...
			if (editStart > regionStart) { // the edit starts after the region's start => no possible overlap of
											// region's start
				top = i - 1;
				if (editStart > regionEnd) { // the edit starts after the region's end => no possible overlap of
												// region's end
					topEnd = top;
				}
			} else {
				if (editEnd < regionStart) { // the edit ends before the region's start => no possible overlap of
												// region's start
					bottom = i + 1;
				} else {
					...
					if (length > 0) {
						...
						// If the edit was a replacement but become an insertion due to the length
						// reduction
						// and if the edit finishes just before the region starts and if there's no line
						// to replace
						...
						if (editLength > 0 && edit.length == 0 && editEnd == regionStart && linesReplaced == 0
							...
							// As the edit starts outside the region, remove first lines from edit string if
							// any
							if (linesReplaced > 0) {
								...
								if (linesCount > 0) {
									...
									if (replacementStart > length
											|| (replacementStart == length && spacesOutsideLength > 0)) {
										edit.offset = -1;
		...
		while (bottom <= topEnd) {
			...
			int editStart = edit.offset;
			int editEnd = editStart + edit.length;
			if (regionEnd < editStart) { // the edit starts after the region's end => no possible overlap of region's
											// end
				topEnd = i - 1;
				...
				break;
			} else if (editEnd <= regionEnd) { // the edit ends before the region's end => no possible overlap of
												// region's end
				bottom = i + 1;
			} else {
				// Count the lines of the edit which are outside the region
				...
				if (length != 0) {
					if (replacementLength != 0) {
						this.edits[this.editsIndex - 1] = new OptimizedReplaceEdit(previousOffset,
								previousLength + length, previousReplacement + replacement);
					} else if (previousLength + length == previousReplacementLength) {
						// check the characters. If they are identical, we can get rid of the previous
						// edit
						...
						this.edits[this.editsIndex - 1] = new OptimizedReplaceEdit(previousOffset,
								previousLength + length, previousReplacement);
					}
				} else {
					if (replacementLength != 0) {
	...
	private int consumeInvalidToken(int end) {
		...
		// In case of invalid unicode character, consume the current backslash character
		// before continuing
	...
	public Alignment createAlignment(int kind, int mode, int tieBreakRule, int count, int sourceRestart,
		...
		if ((this.currentAlignment == null && this.formatter.expressionsDepth >= 0) || (this.currentAlignment != null
				&& this.currentAlignment.kind == Alignment.BINARY_EXPRESSION && (this.formatter.expressionsPos
						& CodeFormatterVisitor.EXPRESSIONS_POS_MASK) == CodeFormatterVisitor.EXPRESSIONS_POS_BETWEEN_TWO)) {
			switch (kind) {
	...
	/*
	 * Returns the index of the comment including the given offset position starting
	...
	/**
	 * Answer next indentation level based on column estimated position (if column
	 * is not indented, then use indentationLevel)
	 */
	...
	private int useAlignmentBreakIndentation(int emptyLinesRules) {
		...
		if ((this.currentAlignment != null || specificEmptyLinesRule)
			...
			if (this.lastNumberOfNewLines == 0 || specificEmptyLinesRule
					|| this.formatter.arrayInitializersDepth >= 0) {
				...
				case PRESERVE_EMPTY_LINES_IN_BINARY_EXPRESSION:
					...
					if ((this.formatter.expressionsPos
							& CodeFormatterVisitor.EXPRESSIONS_POS_MASK) == CodeFormatterVisitor.EXPRESSIONS_POS_BETWEEN_TWO) {
				...
				case PRESERVE_EMPTY_LINES_IN_EQUALITY_EXPRESSION:
					useAlignmentShiftBreakIndentation = this.currentAlignment == null
	...
	private void handleLineTooLongSmartly() {
		...
		while (targetAlignment != null) {
			boolean couldBreak = targetAlignment.tieBreakRule == Alignment.R_OUTERMOST
					|| (!insideStringConcat && insideMessage > 0 && targetAlignment.kind == Alignment.MESSAGE_ARGUMENTS
	...
	/*
	 * Check if there is a NLS tag on this line. If yes, return true, returns false
	 * otherwise.
	 */
			...
			if (commentIndex >= 0 && commentIndex < this.commentPositions.length) {
				...
				if (start < 0) {
					...
					if (lineIndexForComment == index) {
						return CharOperation.indexOf(Scanner.TAG_PREFIX, this.scanner.source, true, start,
								currentLineEnd) != -1;
					}
				}
			}
	...
	private void initializeScanner(long sourceLevel, DefaultCodeFormatterOptions preferences) {
		...
		this.scanner = new Scanner(true, true, false/* nls */, sourceLevel/* sourceLevel */, taskTags,
	...
	private void printBlockComment(boolean isJavadoc) {
		...
		if (this.indentationLevel != 0) {
			if (isJavadoc || !this.formatter.preferences.never_indent_block_comments_on_first_column
					|| !onFirstColumn) {
				indentComment = true;
				printIndentationIfNecessary();
			}
		...
		boolean formatComment = (isJavadoc && (this.formatComments & CodeFormatter.K_JAVA_DOC) != 0)
									...
									case '\t':
										if (this.tabLength != 0) {
											...
											if (reminder == 0) {
												...
												currentIndentation = ((currentIndentation / this.tabLength) + 1)
														* this.tabLength;
											}
										}
										break;
	...
	private boolean printBlockComment(int currentTokenStartPosition, int currentTokenEndPosition) {
		...
		while (!this.scanner.atEnd()) {
			...
			case TerminalTokens.TokenNameWHITESPACE:
				...
				lineNumber = Util.getLineNumber(this.scanner.currentPosition, this.lineEnds,
			...
			case TerminalTokens.TokenNameMULTIPLY_EQUAL:
				if (newLine) {
					...
					scannerLine = Util.getLineNumber(this.scanner.currentPosition, this.lineEnds,
							scannerLine > 1 ? scannerLine - 2 : 0, this.maxLines);
					continue;
				}
				break;
			...
			if (linesGap > max) {
				if (clearBlankLines) {
					// TODO (frederic) see if there's a bug for the unremoved blank line for root
					// tags
			...
			int tokenLength = (this.scanner.atEnd() ? this.scanner.eofPosition : this.scanner.currentPosition)
					- tokenStart;
			hasTokens = true;
			if (!isTokenStar)
				lineHasTokens = true;
			...
			if (lineHasTokens && !firstWord && lastColumn > maxColumn) {
				...
				if ((this.indentationLevel + tokensStringLength + tokenLength) > maxColumn) {
					// there won't be enough room even if we break the line before the buffered
					// tokens
	...
	public void printEndOfCompilationUnit() {
		try {
			// if we have a space between two tokens we ensure it will be dumped in the
			// formatted string
	...
	private void printCodeSnippet(int startPosition, int endPosition, int linesGap) {
		...
		int lastLine = Util.getLineNumber(endPosition, this.lineEnds, firstLine > 1 ? firstLine - 2 : 0, this.maxLines)
				- 1;
		...
		TextEdit edit = CommentFormatterUtil.format2(CodeFormatter.K_UNKNOWN | CodeFormatter.F_INCLUDE_COMMENTS,
				convertedSnippet, 0, this.lineSeparator, options);
		if (edit == null) {
	...
	/*
	 * Main method to print and format comments (javadoc, block and single line
	 * comments)
	 */
			...
			while ((this.currentToken = this.scanner.getNextToken()) != TerminalTokens.TokenNameEOF) {
				...
				case TerminalTokens.TokenNameWHITESPACE:
					...
					if (realTrailing && this.scanner.currentCharacter == '/'
						...
						// for basic trailing comment preceded by a line comment, then it depends on the
						// comments relative position
						// when following comment column (after having been rounded) is below the
						// preceding one,
						...
						if (trailing == BASIC_TRAILING_COMMENT && hasLineComment) {
							...
							int relativeIndentation = currentCommentIndentation
									- this.lastLineComment.currentIndentation;
							if (this.tabLength == 0) {
					...
					// is enough to have an empty line as the line end is included in the comment
					// line...
					// If comments are contiguous, store the white spaces to be able to compute the
					// current comment indentation
					...
					if (realTrailing) {
						...
						// if one or several new lines are consumed, following comments cannot be
						// considered as trailing ones
						if (lines >= 1) {
							if (hasComment) {
						...
						if (lines == 0) {
							...
							addDeleteEdit(tokenStartPosition, whitespacesEndPosition);
						} else if (lines != 0 && (!this.formatter.preferences.join_wrapped_lines
				...
				case TerminalTokens.TokenNameCOMMENT_LINE:
					if (this.useTags && this.editsEnabled) {
						...
						if (foundTaskCount > 0) {
							...
							turnOff = true;
						} else if (this.tagsKind == this.currentToken && CharOperation.fragmentEquals(this.disablingTag,
								this.scanner.source, tokenStartPosition, true)) {
					...
					if (this.useTags && !this.editsEnabled) {
						if (foundTaskCount > 0) {
							...
							this.editsEnabled = CharOperation.fragmentEquals(this.enablingTag, this.scanner.source,
									tokenStartPosition, true);
						}
					}
					break;
				case TerminalTokens.TokenNameCOMMENT_BLOCK:
	...
	void printComment(int kind, String source, int start, int end, int level) {
		...
		// Put back 3.4RC2 code => comment following line as it has an impact on Linux
		// tests
	...
	private void printLineComment() {
		...
		if (this.indentationLevel == 0) {
			...
			if (onFirstColumn && ((includesLineComments
				...
				if (this.lastLineComment.contiguous) {
					// The leading spaces have been set while looping in the printComment(int)
					// method
					int currentCommentIndentation = getCurrentIndentation(this.lastLineComment.leadingSpaces, 0);
					// Keep the current comment indentation when over the previous contiguous line
					// comment
		...
		if (!isNlsTag && includesLineComments && (!onFirstColumn
				|| this.formatter.preferences.comment_format_line_comment_starting_on_first_column)) {
		...
		while (!this.scanner.atEnd()) {
			...
			case TerminalTokens.TokenNameIdentifier:
				if (previousToken == -1 || previousToken == SKIP_FIRST_WHITESPACE_TOKEN) {
					...
					if (CharOperation.equals(identifier, Parser.FALL_THROUGH_TAG, 0, 5/* length of string "$FALL" */)
							&& this.scanner.currentCharacter == '-') {
						try {
					...
					// this was not a valid fall-through tag, hence continue to process the comment
					// normally
					this.scanner.startPosition = startPosition;
			...
			// replace space at the beginning if present
				...
				// insert new line if max line width is reached and a space was previously
				// encountered
				if (spaceStartPosition > 0 && (this.column + tokenLength) > maxColumn) {
					...
					if (lastTokenEndPosition > spaceEndPosition) {
						this.column += lastTokenEndPosition - (spaceEndPosition + 1); // add all previous tokens lengths
																						// since last space
					}
				...
				while (this.column <= this.indentationLevel) {
					if (this.column <= columnForLeadingIndents) {
						if (this.tabLength > 0 && (this.column - 1 + this.tabLength) <= this.indentationLevel) {
							...
							// hence this.column will be incremented and then avoid endless loop (see bug
							// 290905)
	...
	private void printJavadocBlock(FormatJavadocBlock block) {
		...
		if (block.isDescription()) {
			...
			if (!block.isInlined()) {
				boolean indentRootTags = this.formatter.preferences.comment_indent_root_tags
						&& !block.isInDescription();
				int commentIndentationLevel = 0;
				if (indentRootTags) {
					commentIndentationLevel = tagLength + 1;
					boolean indentParamTag = this.formatter.preferences.comment_indent_parameter_description
							&& block.isInParamTag();
					if (indentParamTag) {
		...
		for (int i = 0; i <= maxNodes; i++) {
			...
			int newLines;
			if (i == 0) {
				newLines = this.formatter.preferences.comment_insert_new_line_for_parameter && block.isParamTag() ? 1
						: 0;
				if (nodeStart > (previousEnd + 1)) {
					if (!clearBlankLines || !joinLines) {
						...
						if (clearBlankLines) {
							// clearing blank lines in this block means that break lines should be
							// preserved, hence only keep one new line
							if (newLines > 0)
								newLines = 1;
						}
					}
			...
			previousLine = Util.getLineNumber(previousEnd, this.lineEnds, node.lineStart > 1 ? node.lineStart - 2 : 0,
					this.maxLines);
		}
		this.lastNumberOfNewLines = 0;
	}
		...
		try {
			...
			if (text != null) {
				...
				if (text.isHtmlTag()) {
					...
					length += (this.scanner.atEnd() ? this.scanner.eofPosition : this.scanner.currentPosition)
				...
				default:
					if (!inlined && spacePosition > 0 && (this.column + tokenLength) > maxColumn) {
						...
						if (newLineString == null) {
							...
							newLineString = this.tempBuffer.substring(0, this.tempBuffer.length() - 1); // remove last
																										// space as
																										// buffer will
																										// be inserted
																										// before a
		...
		if (text.isImmutable()) {
			...
			while (!this.scanner.atEnd()) {
				try {
					...
					textLength += (this.scanner.atEnd() ? this.scanner.eofPosition : this.scanner.currentPosition)
							- this.scanner.startPosition;
				} catch (InvalidInputException e) {
					// maybe an unterminated string or comment
					textLength += (this.scanner.atEnd() ? this.scanner.eofPosition : this.scanner.currentPosition)
	...
	void printJavadocComment(int start, int end) {
		...
		try {
			...
			int newLines = (this.formatter.preferences.comment_new_lines_at_javadoc_boundaries
					&& (this.line > currentLine || javadoc.isMultiLine())) ? 1 : 0;
	...
	/*
	 * prints the empty javadoc line between the 2 given positions. May insert new
	...
	private void printJavadocGapLines(int textStartPosition, int textEndPosition, int newLines, boolean clearBlankLines,
		...
		try {
			// If no lines to set in the gap then just insert a space if there's enough room
			// to
			if (newLines == 0) {
				if (output == null) {
			...
			while (!this.scanner.atEnd()) {
				switch (this.scanner.getNextToken()) {
				case TerminalTokens.TokenNameMULTIPLY:
					// we just need to replace each lines between '*' with the javadoc formatted
					// ones
		...
		try {
			...
			for (int idx = 0, max = text.separatorsPtr; idx <= max; idx++) {
				...
				while (textLineStart < lineStart) {
					...
					textLineStart = Util.getLineNumber(this.scanner.currentPosition - 1, this.lineEnds, textLineStart,
							this.maxLines);
				}
			}
		} catch (InvalidInputException iie) {
	...
	/*
	 * Print the gap lines for an immutable block. That's needed to be specific as
	 * the formatter needs to keep white spaces if possible except those which are
	 * indentation ones. Note that in the peculiar case of a two lines immutable tag
	 * (multi lines block), the formatter will join the two lines.
	 */
	...
	private int printJavadocHtmlTag(FormatJavadocText text, FormatJavadocBlock block, boolean textOnNewLine) {
		...
		if (text.depth >= this.javadocHtmlTagBuffers.length) {
			...
			System.arraycopy(this.javadocHtmlTagBuffers, 0,
					this.javadocHtmlTagBuffers = new StringBuffer[text.depth + 6], 0, length);
		}
		...
		for (int idx = 0, ptr = 0; idx <= max || (text.htmlNodesPtr != -1 && ptr <= text.htmlNodesPtr); idx++) {
			...
			if (text.htmlNodesPtr >= 0 && ptr <= text.htmlNodesPtr && end > text.htmlNodes[ptr].sourceStart) {
				...
				this.needSpace = idx > 1 && (previousEnd + 1) < nextStart; // There's no space between text and html tag
																			// or inline block => do not insert space a
																			// the beginning of the text
				printJavadocTextLine(buffer, nextStart, end, block, idx == 0, needIndentation,
						idx == 0/* opening html tag? */ || text.htmlIndexes[idx - 1] != -1);
				linesAfter = 0;
				if (idx == 0) {
			...
			// Replace with current buffer if there are several empty lines between text
			// lines
			...
			if (linesGap > 0) {
				if (clearBlankLines) {
					...
					if (idx == 0 || linesGap > 1 || (idx < max && nodeKind == 1
							&& (text.htmlIndexes[idx - 1] & JAVADOC_TAGS_ID_MASK) != JAVADOC_IMMUTABLE_TAGS_ID)) {
				...
				if (codeEnd > end) {
					if (this.formatter.preferences.comment_format_source) {
						...
						printJavadocGapLines(end + 1, nextStart - 1, newLines,
								false/* clear first blank lines inside <pre> tag as done by old formatter */, false,
								null);
						// Format the code
						printCodeSnippet(nextStart, codeEnd, linesGap);
						// Format the gap lines after the code
						nextStart = (int) text.separators[max];
						printJavadocGapLines(codeEnd + 1, nextStart - 1, 1,
								false/* clear blank lines inside <pre> tag as done by old formatter */, false, null);
						return 2;
					}
				} else {
					nextStart = (int) text.separators[max];
					if ((nextStart - 1) > (end + 1)) {
						...
						printJavadocGapLines(end + 1, nextStart - 1, gapLines,
								false/* never clear blank lines inside <pre> tag */, false, null);
						if (gapLines > 0)
							textOnNewLine = true;
					}
				}
		...
		this.needSpace = !closingTag && max > 0 // not a single or not closed tag (e.g. <br>)
				&& (previousEnd + 1) < nextStart; // There's no space between text and html tag or inline block => do
													// not insert space a the beginning of the text
		printJavadocTextLine(buffer, nextStart, text.sourceEnd, block, max <= 0, needIndentation,
				closingTag/* closing html tag */);
	...
	private void printJavadocText(FormatJavadocText text, FormatJavadocBlock block, boolean textOnNewLine) {
		...
		for (int idx = 0, max = text.separatorsPtr; idx <= max; idx++) {
			...
			printJavadocTextLine(this.javadocTextBuffer, nextStart, end, block,
					idx == 0 || (!joinLines && textOnNewLine)/* first text? */, needIndentation,
		...
		printJavadocTextLine(this.javadocTextBuffer, nextStart, text.sourceEnd, block,
				text.separatorsPtr == -1 /* first text? */, needIndentation, false /* not an html tag */);
			...
			while (!this.scanner.atEnd()) {
				...
				case TerminalTokens.TokenNameWHITESPACE:
					if (tokensBufferLength > 0) {
						boolean shouldSplit = (this.column + tokensBufferLength) > maxColumn // the max length is
																								// reached
								&& !isHtmlTag && (insertSpace || tokensBufferLength > 1) // allow to split at the
																							// beginning only when
																							// starting with an
				...
				boolean shouldSplit = lastColumn > maxColumn // the max length is reached
						...
						&& (tokensBufferLength == 0 || this.javadocTokensBuffer.charAt(tokensBufferLength - 1) != '@'); // avoid
																														// to
																														// split
																														// just
																														// before
				...
				if (shouldSplit) {
					...
					if ((tokensBufferLength > 0
							|| /* (firstColumn+tokenLength) < maxColumn || (insertSpace && */ this.column > firstColumn)
				...
				case TerminalTokens.TokenNameCOMMENT_JAVADOC:
					if (this.useTags && this.editsEnabled) {
						...
						if (foundTaskCount > 0) {
							setEditsEnabled(foundTaskCount);
							turnOff = true;
						} else if (this.tagsKind == this.currentToken && CharOperation.equals(this.disablingTag,
								this.scanner.source, tokenStartPosition, tokenEndPosition + 1)) {
							this.editsEnabled = false;
							turnOff = true;
						}
						if (turnOff) {
					...
					if (this.useTags && !this.editsEnabled) {
						if (foundTaskCount > 0) {
							...
							this.editsEnabled = CharOperation.equals(this.enablingTag, this.scanner.source,
									tokenStartPosition, tokenEndPosition + 1);
						}
					}
					currentTokenStartPosition = this.scanner.currentPosition;
					hasComment = true;
					...
					if (this.useTags && this.editsEnabled) {
						...
						if (foundTaskCount > 0) {
							...
							turnOff = true;
								...
								this.scanner.source, tokenStartPosition, tokenEndPosition)) {
							this.editsEnabled = false;
							turnOff = true;
						}
						if (turnOff) {
	...
	private void printNewLinesBeforeDisablingComment() {
		...
		try {
			...
			// Split the existing edit to keep the change before the beginning of the last
			// line
			// but change the indentation after. Note that at this stage, the add*Edit
			// methods
			...
			if (replacement.length() == 0) {
				...
				this.edits[this.editsIndex - 1] = new OptimizedReplaceEdit(beginningOfLine,
						offset + length - beginningOfLine, indentationString);
			} else {
				...
				if (idx >= 0) {
					...
					this.edits[this.editsIndex - 1] = new OptimizedReplaceEdit(offset, length,
	 ...
	 * the line separator is replaced if necessary, the other white spaces are
	 * untouched.
	 */
				...
				case '\n':
					if (needReplace) {
						if (this.editsIndex == 0 || this.edits[this.editsIndex - 1].offset != start) {
							this.edits[this.editsIndex++] = new OptimizedReplaceEdit(start,
									this.scanner.currentPosition - start, this.lineSeparator);
						}
					}
					foundNewLine = true;
					break;
		...
		try {
			...
			if (expectedTokenType != this.currentToken) {
				throw new AbortFormatting(
						"unexpected token type, expecting:" + expectedTokenType + ", actual:" + this.currentToken);//$NON-NLS-1$//$NON-NLS-2$
			}
	...
	public void printNextToken(int[] expectedTokenTypes, boolean considerSpaceIfAny) {
		...
		try {
			...
			if (Arrays.binarySearch(expectedTokenTypes, this.currentToken) < 0) {
				...
				throw new AbortFormatting("unexpected token type, expecting:[" + expectations.toString() + "], actual:" //$NON-NLS-1$//$NON-NLS-2$
						+ this.currentToken);
			}
			print(this.scanner.currentPosition - this.scanner.startPosition, considerSpaceIfAny);
		} catch (InvalidInputException e) {
	...
	private void resize() {
		System.arraycopy(this.edits, 0, (this.edits = new OptimizedReplaceEdit[this.editsIndex * 2]), 0,
				this.editsIndex);
	}
	...
	/*
	 * Look for the tags identified by the scanner to see whether some of them may
	 * change the status of the edition for the formatter. Do not return as soon as
	 * a match is found, as there may have several disabling/enabling tags in a
	 * comment, hence the last one will be the one really changing the formatter
	 * behavior...
	 */
	...
	public String toString() {
		...
		stringBuffer.append(") - (tabSize = " + this.tabLength + ")")//$NON-NLS-1$//$NON-NLS-2$
				...
				.append("(line = " + this.line + ") - (column = " + this.column + ") - (identationLevel = " //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
						+ this.indentationLevel + ")") //$NON-NLS-1$
				.append(this.lineSeparator)
				.append("(needSpace = " + this.needSpace + ") - (lastNumberOfNewLines = " + this.lastNumberOfNewLines //$NON-NLS-1$ //$NON-NLS-2$
						+ ") - (checkLineWrapping = " + this.checkLineWrapping + ")") //$NON-NLS-1$ //$NON-NLS-2$
				.append(this.lineSeparator)
				.append("==================================================================================") //$NON-NLS-1$
				.append(this.lineSeparator);
		if (this.tabLength > 0) {
